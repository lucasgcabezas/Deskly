...
const taskActions = {
    tasksFromTaskplanner: (taskPlannerId) => {
        return async (dispatch, getState) => {
            try {
                const response = await axios.post('http://localhost:4000/api/task/' + taskPlannerId)
    ...
    addTask: (taskToAdd) => {
        return async (dispatch, getState) => {
            try {
                const response = await axios.post('http://localhost:4000/api/task', taskToAdd)
    ...
    editTask: (taskEditId, taskEdit) => {
        return async (dispatch, getState) => {
            try {
                const response = await axios.post('http://localhost:4000/api/task/' + taskEditId, taskEdit)
    ...
    deleteTask: (taskEditId, taskEdit) => {
        return async (dispatch, getState) => {
            try {
                const response = await axios.post('http://localhost:4000/api/task/' + taskEditId)
